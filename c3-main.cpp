	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			// TODO: (Filter scan using voxel filter)
          	// creating voxelgrid filter object vg
			...
			// TODO: Transform scan so it aligns with ego's actual pose and render that scan
          	PointCloudT::Ptr transformed_scanCloud(new PointCloudT);
          	pcl::transformPointCloud(*cloudFiltered, *transformed_scanCloud, transform_matrix);

			viewer->removePointCloud("scan");
			// TODO: Change `scanCloud` below to your transformed scan
			renderPointCloud(viewer, transformed_scanCloud, "scan", Color(1,0,0) );
